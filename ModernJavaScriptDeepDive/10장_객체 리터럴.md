...
- 프로퍼티 키가 식별자 네이밍 규칙을 준수하지 않는 이름(자바스크립트에서 사용 가능한 유효한 이름)이 아니면 반드시 대괄효 표기법을 사용해야 한다.
...
### 10.9.1. 프로퍼티 축약 표현
- 프로퍼티 값으로 변수를 사용하는 경우 변수 이름과 프로퍼티 키가동일한 이름일 때 프로퍼티 키를 생략할 수 있다.
...
### 10.9.2. 계산된 프로퍼티 이름
- **계산된 프로퍼티 이름: 문자열 또는 문자열로 타입 변환할 수 있는 값으로 평가되는 표현식을 사용해 프로퍼티 키를 동적 으로 생성할 수도 있다.**
- 단, 프로퍼티 키를 사용할 표현식을 대괄호로 묶어야 한다.
...
- 메서드 축약 표현으로 정의한 메서드는 프로퍼티에 할당한 함수와 다르게 동작한다.(26장 예정)
```javascript
// ES6
const obj = {
  name: 'Lee',
