...
- 즉, **🏷️이터러블은 Symbol.iterator를 프로퍼티 키로 사용한 메서드를 직접 구현하거나 프로토타입 체체인을 통해 상속 받은 객체를 말한다.**
<br />
...
```
- 유사 배열 객체는 이터러블이 아닌 일반 객체닫. 따라서 유사 배열 객체에는 Symbol.iterator 메서드가 없기 때문에 for ...of 문으로 순회할 수 없다.
```javascript
...
```
- 단, arguments, NodeList, HTMLCollection은 유사 뱅려 객체이면서 이터러블이다. 배열 또한 ES6에서 이터러블이 도입되면서 Symbol.iterator 메서드를 구현하여 이터러블이 되었다.
- **하지만 모든 유사 배열 객체가 이터러블인 것은 아니다.** 위 예에즤 arrayLike 객체는 유사 배열 객체지만 이터러블이 아니다.
...
- ES6 이전에서의 순회 가능한 데이터 컬렉션(배열, 문자열, 유사 배열 객체 등)은 통일된 규약 없이 각자 나름의 구조를 가지고 for, for ...in, forEach 메서드 등 다양한 방법으로 순회할 수 있었다.
...
```
- 사용자 정의 이터러블은 이터레이션 프로토콜을 준수하도록 Symbol.iterator 메서드를 구현하고 Symbol.oterator 메서드가 next 메서드를 갖는 이터레이터를 반환하도록 한다. 그리고 이터레이터의 next 메서드는 done과 value 프로퍼티를 가지는 이터레이터 리절트 객체를 반환한다. for ...of 문은 done 프로퍼티가 true가 될 때까지 반복하며 done 프로퍼티가 true가 되면 반복을 중지한다.
...
- 앞서 살펴본 fibonacci 이터러블은 내부에 수열의 최대값 max를 가지고 잇따. 이 수열의 최대값은 고정된 값을 ㅗ외부에서 전달한 값으로 변경할 방법이 없다는 아쉬움이 있다.
...
- 무한 이터러블을 생성하는 함수를 정의해보자. 이를 통해 무한 수열을 간단힌 구현할 수 있다.
```javascript
// 무한 이터러블을 생성하는 함수
